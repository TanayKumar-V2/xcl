...
export const useCreatePost=()=>{
    ...
    const handleImagePicker=async(useCamera:boolean=false)=>{
        ...
        if(permissionResult.status!="granted"){
            ...
            Alert.alert("Permission Needed",`Please grant permission to ${source}`)
            return
        }
    ...
    const createPost=()=>{
        if(!content.trim() && !selectedImage){
            Alert.alert("Error","Please add some text or Image before posting")
            return
        }
    ...
    return{
        content,setContent,selectedImage,
