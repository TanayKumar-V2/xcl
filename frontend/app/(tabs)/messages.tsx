...
const MessagesScreen = () => {
    ...
    const [selectedConversations, setSelectedConversations] = useState<ConversationType | null>(null)
    ...
    const openConveration = (conversation: ConversationType) => {
    ...
    const sendMessage = () => {
        if (newMessage.trim() && selectedConversations) {
            setConversationsList((prev) => prev.map((conv) => conv.id === selectedConversations.id ? { ...conv, lastMessage: newMessage, time: "now" } : conv))
            setNewMessage("")
            Alert.alert("Mesaage Sent", `Your message has been sent to ${selectedConversations.user.name}`)
        }
    }
        ...
        <SafeAreaView className='flex-1 bg-white' edges={["top"]}>
            ...
            <ScrollView className='flex-1' showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 100 + insets.bottom }}>
                {conversationsList.map(conversation => (
                    <TouchableOpacity key={conversation.id} className='flex-row items-center p-4 border-b border-gray-50 active:bg-gray-50 ' onPress={() => openConveration(conversation)} onLongPress={() => deleteConversation(conversation.id)}>
                        <Image
                ...
                {selectedConversations && (
                    <SafeAreaView className="flex-1">
                        ...
                        <View className="flex-row items-center px-4 py-3 border-b border-gray-100">
                            ...
                            <Image
                                source={{ uri: selectedConversations.user.avatar }}
                                className="size-10 rounded-full mr-3"
                            />
                            <View className="flex-1">
                                <View className="flex-row items-center">
                                    <Text className="font-semibold text-gray-900 mr-1">
                                        {selectedConversations.user.name}
                                    </Text>
                                    {selectedConversations.user.verified && (
                                        <Feather name="check-circle" size={16} color="#1DA1F2" />
                                    )}
                                </View>
                                <Text className="text-gray-500 text-sm">@{selectedConversations.user.username}</Text>
                            </View>
                        </View>
                        ...
                        <ScrollView className="flex-1 px-4 py-4">
                            <View className="mb-4">
                                <Text className="text-center text-gray-400 text-sm mb-4">
                                    This is the beginning of your conversation with {selectedConversations.user.name}
                                </Text>

                                {/* Conversation Messages */}
                                {selectedConversations.messages.map((message) => (
                                    <View
                                        key={message.id}
