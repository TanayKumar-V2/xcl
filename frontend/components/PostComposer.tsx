  ...
  return (
    <View className='border-b border-gray-100 p-4 bg-white'>
        ...
        <View className='mt-3 ml-15'>
          <View className='relative'>
            <Image
              ...
              resizeMode='cover'
            />
            <TouchableOpacity className='absolute top-2 right-2 w-8 h-8 bg-black bg-opacity-60 rounded-full items-center justify-center' onPress={removeImage}>
      ...
      <View className='flex-row justify-between items-center mt-3'>
        ...
        <TouchableOpacity className={`px-6 py-2 rounded-full ${content.trim() || selectedImage ? "bg-blue-500":"bg-gray-300"}`} onPress={createPost}  disabled={isCreating || !(content.trim()||selectedImage)}>
          {isCreating ? (
